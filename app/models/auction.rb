class Auction < ActiveRecord::Base
  ...
  scope :hot, active.where(["(CURRENT_TIMESTAMP - end_time) < interval '20 minutes'"])
  scope :recently_updated, where("updated_at > CURRENT_TIMESTAMP - interval '4 hours'").with_end

  enum_attr :gender, %w(male female)
  enum_attr :auction_status, %w(novice, ^standard, advanced)

  state_machine :state, :initial => :upcoming do
    #state :upcoming
    #state :open
    #state :cancelled
    ...
    event :end do
      transition [:open, :cancelled, :awaiting_payment] => :closed, :if => :end_auction
    end

    event :cancel do
      transition [:open, :awaiting_payment, :cancelled, :closed] => :cancelled, :if => :end_auction
    end

    # Takes 1 argument: Number of seconds to reopen for.
    event :reopen do
      transition [:cancelled, :awaiting_payment, :closed] => :open, :if => :reopen_auction
    end

    event :start do
      transition :upcoming => :open, :if => :open_auction
    end
  end

  # State machine overloads
  def reopen(*args)
    returning(super) do
      raise("Failed to reopen auction.") unless running?
    end
  end
